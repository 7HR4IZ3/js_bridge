const { proxymise, makePyClass } = require("./utils.js")
...
const BaseChainableBridgeProxyHandler = {
  get: function(target, property) {
    ...
    if (property == "cls") {
      return (makePyClass(target.$$main))
    }
    ...
    if (typeof property === 'symbol') {
      if (property === Symbol.iterator) {
        ...
        return function * iter () {
          for (let i = 0; i < 100; i++) {
            const next = new BaseChainableBridgeProxy([...target.callstack, i])
    ...
    const next = new BaseChainableBridgeProxy(target.callstack, target.data);
...
//       this.__main = new Proxy(this, this.__handler);
//       return this.__main;
//     }
// }

const BaseChainableBridgeProxy = (conn, data, handler=BaseChainableBridgeProxyHandler) => {
  let t = new Intermediate([], {
    server: conn,
    data: data
  });
  let ret = new Proxy(t, handler);
  t.data.$$main = ret;
  return ret;
}
